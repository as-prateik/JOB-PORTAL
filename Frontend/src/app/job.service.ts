...
export class JobService {
  ...
  updateApplicantStatus(jobId: string, employeeId: string, status: string,token: string): Observable<any> {
        const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`,
    });
    return this.http.patch(
      `${this.apiUrl}/${jobId}/applicants/${employeeId}/status`,
      { status },  { headers }
    );
  }
  ...
  approveApplicantTransfer(jobId: string, employeeId: string) {
    return this.http.patch(
      `/api/jobs/${jobId}/applicants/${employeeId}/approve`,
      {}
    );
  }
  ...
  getOnDemandCertifications(token: string): Observable<any> {
    return this.http.get<{
      rankedCertifications: { certification: string; count: number }[];
