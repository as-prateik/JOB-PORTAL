...
exports.updateApplicantStatus = async (req, res) => {
  try {
    console.log("Updating applicant status with data:", req.params);
    const { jobId, employeeId } = req.params;

    const { status } = req.body; // 'selected' | 'rejected'

    console.log("jobId is ", jobId, " employeeId is ", employeeId, " status is ", status);
    
    const job = await Job.findById(jobId);
    console.log("job is",job.postedBy);
    console.log("user is ",req.user.employeeId);
    ...
    if (!applicant) {
      return res
        .status(404)
        .json({ message: "Applicant not found for this job" });
    }
...
exports.approveApplicantTransfer = async (req, res) => {
  try {
    ...
    if (user.reportsTo !== req.user.userId) {
      return res
        .status(403)
        .json({ message: "You are not authorized to approve this transfer" });
    }
    ...
    if (!applicant || applicant.status !== "selected") {
      return res
        .status(400)
        .json({ message: "Applicant not selected or not found" });
    }
    ...
    res.json({
      message: "Transfer approved. Employee now reports to new manager.",
    });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
