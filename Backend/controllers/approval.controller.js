...
exports.createTransferApproval = async (req, res) => {
  ...
  try {
    ...
    const toManagerusername = await User.findOne({ employeeId: employeeId }).then(user => user.reportsTo); // Get the new manager's ID from the employee's reportsTo field
    ...
    if (!employeeId || !toManagerId || !jobId||!fromManagerId) {
      ...
      console.log("required fields",employeeId,toManagerId,jobId,fromManagerId)
...
exports.updateApprovalStatus = async (req, res) => {
  ...
  try {
    ...
    let x = await TransferApproval.findOne({ jobId });
